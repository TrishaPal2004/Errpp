...
const ERPDashboard = () => {
  ...
  const fetchRealTimeData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        " https://erp-back-pl3q.onrender.com/api/erp-data",
        {
          method: "POST",
          headers: {
  ...
  const connectToDatabase = async () => {
    ...
    try {
      ...
      const response = await fetch(
        " https://erp-back-pl3q.onrender.com/api/test-connection",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const processDataForERP = (forecastData) => {
    ...
    const currentWeekData = forecastData.filter(
      (item) => parseInt(item.week) === currentWeek
    );
    ...
    const factories = Object.entries(dcSummary).map(([dc, data]) => {
      const totalDemand = _.sumBy(
        data,
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const weeklyAvg =
        totalDemand / _.uniq(data.map((item) => parseInt(item.week))).length;
      const uniqueSkus = _.uniq(data.map((item) => item.sku));
      const CheeseDemand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Cheese") && item.week === currentWeek - 2
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const SnacksDemand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Snacks") && item.week === currentWeek - 2
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const BakeryDemand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Bakery") && item.week === currentWeek - 2
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const FrozenDemand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Frozen") && item.week === currentWeek
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const BeveragesDemand = _.sumBy(
        data.filter(
          (item) =>
            item.sku.includes("Beverages") && item.week === currentWeek - 2
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      return {
    ...
    const suppliers = Object.entries(skuAnalysis).map(([sku, data], index) => {
      ...
      const totalSupplyNeeded = _.sumBy(
        data,
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
    ...
    const retailers = Object.entries(dcSummary).map(([dc, data]) => {
      ...
      const avgDemand = _.meanBy(
        data,
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      // const skuCount = _.uniq(data.map(item => item.sku)).length;
      const demandTrend =
        currentWeekDemand > avgDemand ? "increasing" : "stable";
      const Cheese_Demand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Cheese") && item.week === currentWeek
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const Snacks_Demand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Snacks") && item.week === currentWeek
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const Bakery_Demand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Bakery") && item.week === currentWeek
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const Beverages_Demand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Beverages") && item.week === currentWeek
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      );
      const Frozen_Demand = _.sumBy(
        data.filter(
          (item) => item.sku.includes("Frozen") && item.week === currentWeek
        ),
        (item) => parseFloat(item.forecast_ai_qty) || 0
      ...
      return {
        ...
        stock_status:
          currentWeekDemand > avgDemand * 1.2 ? "critical" : "adequate",
        Cheese_Demand,
        Snacks_Demand,
        Bakery_Demand,
        Beverages_Demand,
    ...
    const totalCurrentWeekDemand = _.sumBy(
      currentWeekData,
      (item) => parseFloat(item.forecast_ai_qty) || 0
    );
    const totalSkus = _.uniq(forecastData.map((item) => item.sku)).length;
    const totalWeeks = _.uniq(
      forecastData.map((item) => parseInt(item.week))
    ).length;
    ...
    const stats = {
      ...
      avgDemandPerSku:
        totalSkus > 0 ? Math.round(totalCurrentWeekDemand / totalSkus) : 0,
    };
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
      ...
      >
        ...
        <button
          ...
          onMouseLeave={(e) => {
            if (connectionStatus !== "connecting")
              e.currentTarget.style.backgroundColor = "#2563EB"; // reset
          }}
        >
        ...
        <button
          ...
          onMouseEnter={(e) => {
            if (connectionStatus === "connected")
              e.currentTarget.style.backgroundColor = autoRefresh
                ? "#B91C1C"
                : "#15803D"; // red-700 / green-700
          }}
          onMouseLeave={(e) => {
            if (connectionStatus === "connected")
              e.currentTarget.style.backgroundColor = autoRefresh
          ...
          {data.length === 0 ? (
            ...
            >
              ...
              <p style={{ fontSize: "0.875rem" }}>
                Connect to database to fetch real-time data
              </p>
            </div>
          ) : (
            data.map((item, index) => (
        ...
        >
          <h1
            style={{
              fontSize: "2.5rem",
              fontWeight: "bold",
              background: "linear-gradient(90deg, #00c6ff, #2eff70, #facc15)",
          ...
          Object.keys(summaryStats).length > 0 && (
            ...
            >
              ...
              <StatCard
                icon={
                  <Factory
                    style={{ width: 28, height: 28, color: "#7c3aed" }}
                  />
                }
              ...
              <StatCard
                icon={
                  <Truck style={{ width: 28, height: 28, color: "#ea580c" }} />
                }
              ...
              <StatCard
                icon={
                  <Store style={{ width: 28, height: 28, color: "#dc2626" }} />
                }
